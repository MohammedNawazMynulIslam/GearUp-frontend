  ...
  return (
    <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5">
        {sections.map((section, index) => (
          <div key={index} className={cn(index > 0 && "mt-6")}>
            {section.label && (
              <p className="px-3 pb-2 text-[0.68rem] font-bold tracking-wider text-muted-foreground uppercase">
                {section.label}
              </p>
            )}
      ...
      <div className="shrink-0 border-t border-sidebar-border p-4">
        <div className="mb-3 flex min-w-0 items-center gap-2.5 rounded-lg bg-muted/60 p-2">
          <Avatar size="sm">
...
export default function DashboardSidebar() {
  ...
  return (
    <>
      <header className="sticky top-14 z-30 flex h-14 items-center gap-2 border-b border-border bg-card/95 px-4 shadow-sm backdrop-blur lg:hidden">
        <Button
          variant="ghost"
          size="icon-sm"
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-72 max-w-[86vw] transform border-r border-sidebar-border bg-sidebar shadow-xl transition-transform duration-200 ease-in-out lg:hidden",
      ...
      <aside className="fixed top-14 bottom-0 left-0 z-20 hidden w-72 border-r border-sidebar-border bg-sidebar lg:block">
        <SidebarContent />
      </aside>
    </>
  )
